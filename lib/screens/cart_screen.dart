        ...
        child: ci.item.imageAsset.isNotEmpty
          ? Image.asset(ci.item.imageAsset, width: 64, height: 64, fit: BoxFit.cover,
              errorBuilder: (_, __, ___) => Container(
                ...
                child: Center(child: Text(ci.item.categoryEmoji, style: const TextStyle(fontSize: 28)))))
          : Container(width: 64, height: 64, color: AppColors.cardBg,
              child: Center(child: Text(ci.item.categoryEmoji, style: const TextStyle(fontSize: 28)))),
      ),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(ci.displayName, style: const TextStyle(fontFamily:'Poppins', fontSize:13, fontWeight:FontWeight.w600, color:AppColors.textPrimary)),
        Text('Rs. ${ci.unitPrice.toInt()} each', style: const TextStyle(fontFamily:'Poppins', fontSize:11, color:AppColors.textSecondary)),
...
class _OrderSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Order Summary', style: TextStyle(fontFamily:'Poppins', fontSize:15, fontWeight:FontWeight.w700, color:AppColors.textPrimary)),
      ...
      if (cart.subtotal < AppConstants.minDelivery)
        Padding(
          ...
          child: Text('Add Rs. ${(AppConstants.minDelivery - cart.subtotal).toInt()} more to qualify for delivery',
...
class _PaymentCardState extends State<_PaymentCard> {
  String _selected = AppConstants.payCOD;
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16),
      boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 6)]),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Payment Method', style: TextStyle(fontFamily:'Poppins', fontSize:15, fontWeight:FontWeight.w700)),
      const SizedBox(height: 12),
      ..._methods.map((m) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: _selected == m.$3 ? AppColors.accent : AppColors.divider,
              width: _selected == m.$3 ? 2 : 1)),
          child: Row(children: [
            Icon(m.$2, color: _selected == m.$3 ? AppColors.accent : AppColors.textSecondary, size: 20),
            const SizedBox(width: 12),
            Text(m.$1, style: TextStyle(fontFamily:'Poppins', fontSize:13, fontWeight:FontWeight.w500,
              color: _selected == m.$3 ? AppColors.textPrimary : AppColors.textSecondary)),
            const Spacer(),
            if (_selected == m.$3)
...
class _PlaceOrderBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: ElevatedButton(
      ...
      child: Obx(() => Text('Place Order  •  Rs. ${cart.total.toInt()}')),
    ),
  );
}
...
class _EmptyCart extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const Text('Your cart is empty', style: TextStyle(fontFamily:'Poppins', fontSize:18, fontWeight:FontWeight.w700, color:AppColors.textPrimary)),
      const SizedBox(height: 6),
      const Text('Add items from the menu to get started', style: TextStyle(fontFamily:'Poppins', fontSize:13, color:AppColors.textSecondary)),
